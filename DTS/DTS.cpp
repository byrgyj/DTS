// DTS.cpp : Defines the entry point for the console application.
//
...
void lsmash_bs_free( lsmash_bs_t *bs )
 ...
 void lsmash_bs_alloc( lsmash_bs_t *bs, uint64_t size )
 {
     ...
     uint8_t *data;
     if( !bs->data )
         data = (uint8_t*)malloc( alloc );
     else
         data = (uint8_t*)realloc( bs->data, alloc );
     if( !data )
     {
         lsmash_bs_free( bs );
         bs->error = 1;
...
uint8_t lsmash_dts_get_stream_construction( lsmash_dts_construction_flag flags ){
     static const int construction_info[DTS_MAX_STREAM_CONSTRUCTION  + 1] =
         {
             0,
 ...
 void dts_update_specific_param( dts_info_t *info )
 {
     ...
     /* StreamConstruction */
     param->StreamConstruction = lsmash_dts_get_stream_construction((lsmash_dts_construction_flag) info->flags );
     /* CoreLFEPresent */
...
int dts_get_channel_count_from_channel_layout( uint16_t channel_layout )
 ...
 int dts_parse_extension_substream( dts_info_t *info, uint8_t *data, uint32_t data_length )
 {
     ...
     if( info->extension.bStaticFieldsPresent )
     {
         ...
         int nuActiveExSSMask[8];
         //int nuActiveExSSMask = new int[nuNumAudioPresnt];
         for( int nAuPr = 0; nAuPr < nuNumAudioPresnt; nAuPr++ )
             nuActiveExSSMask[nAuPr] = (int)dts_bits_get( bits, nExtSSIndex + 1, &bits_pos );     /* nuActiveExSSMask[nAuPr]       (nExtSSIndex + 1) */
         //int nuActiveAssetMask[nuNumAudioPresnt][nExtSSIndex + 1];
         int nuActiveAssetMask[8][2 + 1];
         for( int nAuPr = 0; nAuPr < nuNumAudioPresnt; nAuPr++ )
 ...
 int lsmash_setup_dts_specific_parameters_from_frame( lsmash_dts_specific_parameters_t *param, uint8_t *data, uint32_t data_length )
 {
     ...
     dts_info_t  handler;
     memset(&handler, 0, sizeof(dts_info_t));
...
{
    //FILE *f = fopen("data.amp4", "rb");
    FILE *f = fopen("data.dts", "rb");
    if (f == NULL) {
        return -1;
    }

    //fseek(f, 16, SEEK_SET);
    int32_t dataLength = 1024*1024;
    uint8_t *buffer = new uint8_t[dataLength + 1];
    fread(buffer, 1, dataLength, f);

    int32_t index = 0;
    while(index < dataLength-4) {
        uint32_t syncword = LSMASH_4CC( buffer[index], buffer[index+1], buffer[index+2], buffer[index+3] );
    ...
    lsmash_setup_dts_specific_parameters_from_frame(&param, buffer + index, dataLength);
